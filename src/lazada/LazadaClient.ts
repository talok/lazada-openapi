...
export class LazadaClient {
  ...
  private readonly region: LazadaRegion;
  private readonly apiBaseUrl: string;
  private readonly authBaseUrl: string;
  private readonly timeoutMs: number;
  private readonly tokenStore?: import("../types.js").TokenStore;
  ...
  async exchangeAuthCode(
    ...
    if (redirectUri) data.set("redirect_uri", redirectUri);
    ...
    const token: LazadaTokenResponse = {
  ...
  async refreshAccessToken(
    ...
    const newToken: LazadaTokenResponse = {
      access_token: payload.access_token,
      refresh_token: payload.refresh_token,
      expires_in: Number(payload.expires_in ?? 0),
      account_user_id: payload.account_user_id,
  ...
  async request<T = unknown>(
    ...
    const query: LazadaRequestParams = {
      ...params,
      app_key: this.appKey,
      sign_method: "sha256",
      timestamp,
    ...
    const finalQuery = { ...query, sign: signature } as Record<string, any>;
  ...
  private isTokenExpiredSoon(expiresAtMs?: number): boolean {
